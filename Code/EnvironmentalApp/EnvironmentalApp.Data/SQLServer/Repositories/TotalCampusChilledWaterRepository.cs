...
{
    public class TotalCampusChilledWaterRepository:BaseRepository, Core.Data.SQLServer.IChilledWaterRepository
    {
        ...
        public int Create(List<Core.Models.ChilledWater> entityList)
        {
            try
            {
                using (var ctx = new EnergyDataContext(ConnString))
                {
                    var totalCampusChilledWaterList = new List<ChilledWater>();
                    for (int i = 0; i < totalCampusChilledWaterList.Count; i++)
                    {
                        ctx.TOTAL_CAMPUS_CHILLED_WATER.Add(totalCampusChilledWaterList[i]);
